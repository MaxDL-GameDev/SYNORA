...
{
    public sealed class CreatureRestoringStateTests
    {
        ...
        public void Enter_StopsMovement()
        {
            ...
            var state = new CreatureRestoringState(1.25f);
            state.Enter(ctx);
        ...
        [Test]
        public void RequestsRestored_WhenTimerCompletes()
        {
            var ctx = Build(out _);
            var state = new CreatureRestoringState(1.0f);
            state.Enter(ctx);
            Assert.IsNull(state.Tick(ctx, 0.5f));
            Assert.AreEqual(CreatureStateId.Restored, state.Tick(ctx, 0.5f)); // 1.0 >= 1.0
        }
        ...
        public void Completes_OnExactDuration()
        {
            ...
            var state = new CreatureRestoringState(1.0f);
            state.Enter(ctx);
            Assert.AreEqual(CreatureStateId.Restored, state.Tick(ctx, 1.0f));
        }
        ...
        public void Enter_RestartsTimer()
        {
            ...
            var state = new CreatureRestoringState(1.0f);
            state.Enter(ctx);
            Assert.AreEqual(CreatureStateId.Restored, state.Tick(ctx, 1.0f)); // complete
        ...
        {
            var ctx = Build(out _);
            var state = new CreatureRestoringState(0f);
            state.Enter(ctx);
            Assert.AreEqual(CreatureStateId.Restored, state.Tick(ctx, 0.016f));
        }
        ...
        public void Tick_BeforeEnter_ReturnsNull()
        {
            var ctx = Build(out _);
            var state = new CreatureRestoringState(1.0f);
            Assert.IsNull(state.Tick(ctx, 1.0f), "No timer before Enter; must be safe.");
        }
    }
}
